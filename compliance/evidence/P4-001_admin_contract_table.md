...
| `InjectEscZone` | `/admin/injectdata/esc_zone` | (unspecified) | JSON body | persist injection (injectdata/esc_zone) | thin | — |
...
| `InjectFss` | `/admin/injectdata/fss` | A dictionary with a single key-value pair where the key is "record" and the valu | empty_200 | persist injection (injectdata/fss) | thin | FPR,HBT,MCP,SIQ |
...
| `InjectSasAdministratorRecord` | `/admin/injectdata/sas_admin` | A dictionary with a single key-value pair where the key is "record" and the valu | empty_200 | persist injection (injectdata/sas_admin) | thin | — |
...
| `InjectWisp` | `/admin/injectdata/wisp` | A dictionary with two key-value pairs where the keys are "record" and "zone" wit | empty_200 | persist injection (injectdata/wisp) | thin | FPR,GPR,HBT,MCP,SIQ |
| `InjectZoneData` | `/admin/injectdata/zone` | A dictionary with a single key-value pair where the key is "record" and the valu | zone id JSON | persist injection (injectdata/zone) | thin | FAD,GRA,MCP,PCR,SIQ |
